...
--[[
EPS is meant to help with Lua's floating point math in simple corner cases
like almostEquals(1.1-0.1, 1), which may not work as-is (e.g. on numbers
with rational binary representation) if the user doesn't provide some
explicit error margin. The default margin used by almostEquals() in such
cases is EPS; and since Lua may be compiled with different numeric
precisions (single vs. double), we try to select a useful default for it
dynamically.

Note: If the initial value is not acceptable, it can be changed by the user
to better suit specific needs. See also:
https://en.wikipedia.org/wiki/Machine_epsilon
]]
M.EPS = 2^-52 -- = machine epsilon for "double", ~2.22E-16
if math.abs(1.1 - 1 - 0.1) > M.EPS then -- rounding error is above EPS, assume single precision
...
M.USAGE=[[Usage: lua [options] [testname1 [testname2] ... ]
...
Options:
  ...
  testname1, testname2, ... : tests to run in the form of
     testFunction, TestClass or TestClass.testMethod
...
I have seen the case where running a big suite of test cases and one of them
would perform a os.exit(0), making the outside world think that the full
test suite was executed successfully.

This is an attempt to mitigate this problem: we override os.exit() to now
let a test exit the framework while we are running. When we are not running,
it behaves normally.
]]
M.oldOsExit = os.exit
os.exit = function(...)
...
local function strMatch(s, pattern, start, final )
    -- return true if s matches completely the pattern from index start to end
...
local function prettystr_sub(v, indentLevel, printTableRefs, cycleDetectTable )
    ...
    elseif "number" == type_v then
        ...
        if v ~= v then return "#NaN" -- "not a number"
        end
        if v == math.huge then return "#Inf" -- "infinite"
        end
        if v == -math.huge then return "-#Inf"
        end
        if _VERSION == "Lua 5.3" then
            local i = math.tointeger(v)
            if i then return tostring(i)
            end
        end
    end
    return tostring(v)
...
function M.adjust_err_msg_with_iter( err_msg, iter_msg )
    --[[ Adjust the error message err_msg: trim the FAILURE_PREFIX or SUCCESS_PREFIX information
    if needed, add the iteration message if any and return the result.
    err_msg: string, error message captured with pcall
    iter_msg: a string describing the current iteration ("iteration N") or nil if there is no iteration in this test.
    Returns: (new_err_msg, test_status)
        new_err_msg: string, adjusted error message, or nil in case of success
        test_status: M.NodeStatus.FAIL, SUCCESS or ERROR according to the information contained in the error message.
    ]]
    if iter_msg then
        iter_msg = iter_msg..', '
    else
...
local function tryMismatchFormatting( table_a, table_b, doDeepAnalysis, margin )
    --[[ Prepares a nice error message when comparing tables, performing a deeper analysis.
    Arguments:
...
local function mismatchFormattingMapping( table_a, table_b, doDeepAnalysis )
    --[[ Prepares a nice error message when comparing tables which are not pure lists, performing a deeper analysis.
    Returns: {success, result}
    * success: false if deep analysis could not be performed
        in this case, just use standard assertion message
    * result: if success is true, a multi-line string with deep analysis of the two lists
...
local function mismatchFormattingPureList( table_a, table_b, margin )
    --[[ Prepares a nice error message when comparing tables which are lists, performing a deeper analysis.
...
local function prettystrPairs(value1, value2, suffix_a, suffix_b)
    --[[ This function helps with the recurring task of constructing the
    "expected vs. actual" error messages. It takes two arbitrary values and
    formats corresponding strings with prettystr(). To keep the (possibly
    complex) output more readable in case the resulting strings contain line
    breaks, they get automatically prefixed with additional newlines.
    Both suffixes are optional (default to empty strings), and get appended
    to the "value1" string. "suffix_a" is used if line breaks were
...
local function _table_tostring( tbl, indentLevel, printTableRefs, cycleDetectTable )
    ...
    if mt and mt.__tostring then
        ...
        if type(result) ~= 'string' then
            return string.format( '', prettystr(result) )
        end
        result = strsplit( '\n', result )
        ...
        for k, v in sortedPairs( tbl ) do
            -- key part
            if k == seq_index then
                -- for the sequential part of tables, we'll skip the "=" output
                entry = ''
...
functions for easier management of the table. The "cached" method queries the
state of a specific (actual,expected) pair; and the "store" method sets this
state to the given value. The state of pairs not "seen" / visited is assumed
to be `nil`. ]]
...
local function _is_table_equals(actual, expected, cycleDetectTable, marginForAlmostEqual)
    ...
    If argument marginForAlmostEqual is suppied, number comparison is done using alomstEqual instead of strict equality.
...
function M.assertNotAlmostEquals( actual, expected, margin, extra_msg_or_nil )
    ...
    if M.almostEquals(actual, expected, margin) then
        ...
        fail_fmt(2, extra_msg_or_nil, 'Values are almost equal\nActual: %s, expected: %s' ..
            ', delta %s below margin of %s', actual, expected, delta, margin)
    end
end
...
--[[ Add type assertion functions to the module table M. Each of these
functions takes a single parameter "value", and checks that its Lua type
matches the expected string (derived from the function name):
M.assertIsXxx(value) -> ensure that type(value) conforms to "xxx" ]]
for _, funcName in ipairs( {'assertIsNumber', 'assertIsString', 'assertIsTable',
    'assertIsBoolean', 'assertIsFunction', 'assertIsUserdata', 'assertIsThread'} ) do
...
--[[ Add non-type assertion functions to the module table M. Each of these
functions takes a single parameter "value", and checks that its Lua type
differs from the expected string (derived from the function name):
M.assertNotIsXxx(value) -> ensure that type(value) is not "xxx" ]]
for _, funcName in ipairs( {'assertNotIsNumber', 'assertNotIsString', 'assertNotIsTable',
    'assertNotIsBoolean', 'assertNotIsFunction', 'assertNotIsUserdata', 'assertNotIsThread'} ) do
...
function M.wrapFunctions()
    ...
    -- so just do nothing !
    io.stderr:write[[Use of WrapFunctions() is no longer needed. Just prefix your test function names with "test" or "Test" and they will be picked up and run by LuaUnit.
]]
end
...
function JUnitOutput:endSuite()
    ...
    self.fd:write(string.format( ' <testsuite tests="%d" timestamp="%s" time="%0.3f" errors="%d" failures="%d" skipped="%d" name="LuaUnit Tests">\n',
        self.result.runCount, self.result.startIsodate, self.result.duration,
        self.result.errorCount, self.result.failureCount, self.result.skippedCount ))
...
Tests run: 2, Failures: 1, Errors: 0, Skipped: 0, Time elapsed: 0.03 sec <<< FAILURE!
...
function M.LuaUnit.splitClassMethod(someName)
    ...
    nil, someName instead (the latter being unchanged). This convention thus
    also replaces the older isClassMethod() test: You just have to check for
    a non-nil className (return) value. ]]
...
function M.LuaUnit.version()
    print('LuaUnit v'..M.VERSION..' by Philippe Fremy ')
    os.exit(0)
end
...
function NodeStatus:statusXML()
    if self:isError() then
        return table.concat( {
            ' <error message="'..xmlEscape(self.msg)..'">\n',
            '<![CDATA['..xmlCDataEscape(self.stackTrace)..']]>\n'}) -- CDATA
    elseif self:isFailure() then
        return table.concat( {
            ' <failure message="'..xmlEscape(self.msg)..'">\n',
            '<![CDATA['..xmlCDataEscape(self.stackTrace)..']]>\n'}) -- CDATA
    elseif self:isSkipped() then
        return table.concat({' <skipped message="', xmlEscape(self.msg),'"/>\n' } )
    end
...
local function conditional_plural(number, singular)
    -- returns a grammatically well-formed string "%d "
    local suffix = ''
    if number ~= 1 then
        -- use plural
...
function M.LuaUnit:updateStatus( err )
    ...
    In such case, it's a good idea to report both a failure and an error in the test suite.
    This is what Python unittest does for example.
    However, it mixes up counts so need to be handled carefully: for example, there could be
    more (failures + errors) count that tests.
...
function M.LuaUnit:internalRunSuiteByNames( listOfName )
    --[[
    Run LuaUnit with a list of generic names, coming either from command-line or from global namespace analysis.
    Convert the list into a list of (name, valid instances (table or function)) and calls internalRunSuiteByInstances.
    ]]
...
function M.LuaUnit:unregisterSuite()
    -- unregister the current instance from our global array of instances
    -- print('<- Unregister suite')
    for i,v in ipairs(M.LuaUnit.instances) do
        if v == self then
            table.remove( M.LuaUnit.instances, i )
            return
        end
    end
end
...
function M.LuaUnit:runSuite(...)
    ...
    local cmdLineOptions = self.parseCmdLine( cmdLine )

    -- configure runner from command line options
    self.verbosity = cmdLineOptions.verbosity or self.verbosity
    self.quitOnError = cmdLineOptions.quitOnError
    self.quitOnFailure = cmdLineOptions.quitOnFailure
    self.shuffle = cmdLineOptions.shuffle
    self.exeRepeat = cmdLineOptions.exeRepeat
    self.patternIncludeFilter = cmdLineOptions.pattern
    self.fname = cmdLineOptions.fname

    -- configure output from environment variable first, then from command line
    local outputType = os.getenv('LUAUNIT_OUTPUT')
    if outputType then
        self:setOutputType(outputType, os.getenv('LUAUNIT_JUNIT_FNAME'))
    end
    if cmdLineOptions.output then
        self:setOutputType(cmdLineOptions.output, cmdLineOptions.fname)
    end

    -- select tests to run
    if cmdLineOptions.testNames then
        testNames = cmdLineOptions.testNames
    else
        testNames = self.collectTests()
    end

    self:internalRunSuiteByNames( testNames )
...
function M.LuaUnit:runSuiteByNames( listOfNames, cmdLineOptions )
    -- Run an explicit list of tests, provided as a list of strings
    -- cmdLineOptions is an optional table of options, similar to what is returned by parseCmdLine()
    self:registerSuite()
    cmdLineOptions = cmdLineOptions or {}
    self.patternIncludeFilter = cmdLineOptions.pattern
    self:internalRunSuiteByNames( listOfNames )
    self:unregisterSuite()
    return self.result.notSuccessCount
end

function M.LuaUnit:runSuiteByInstances( listOfNameAndInst, cmdLineOptions )
    -- Run an explicit list of tests, provided as a list of {name, instance}
    -- cmdLineOptions is an optional table of options, similar to what is returned by parseCmdLine()
    self:registerSuite()
    cmdLineOptions = cmdLineOptions or {}
    self.patternIncludeFilter = cmdLineOptions.pattern
...
M.runTest = M.LuaUnit.run

-- When run from the command line, automatically run the tests
-- But do not run if we are included in another script
-- http://stackoverflow.com/questions/499952/how-to-get-the-path-of-the-running-script-in-lua
local is_main_script = not pcall(debug.getlocal, 2, 1)
if is_main_script then
    local runner = M.LuaUnit.new()
    local status = runner:runSuite()
    os.exit(status)
end
